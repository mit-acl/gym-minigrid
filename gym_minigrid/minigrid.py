...
class Goal(WorldObj):
    def __init__(self, color=None):
        WorldObj.__init__(self, 'goal', color=color)
...
class Floor(WorldObj):
    ...
    def __init__(self, color='blue'):
        WorldObj.__init__(self, 'floor', color=color)
...
class Wall(WorldObj):
    def __init__(self, color='grey'):
        WorldObj.__init__(self, 'wall', color=color)
...
class Door(WorldObj):
    def __init__(self, color, is_open=False):
        WorldObj.__init__(self, 'door', color=color)
...
class LockedDoor(WorldObj):
    def __init__(self, color, is_open=False):
        super(LockedDoor, self).__init__('locked_door', color=color)
        self.is_open = is_open
...
class Key(WorldObj):
    def __init__(self, color='blue'):
        super(Key, self).__init__('key', color=color)
...
class Ball(WorldObj):
    def __init__(self, color='blue'):
        super(Ball, self).__init__('ball', color=color)
...
class Box(WorldObj):
    def __init__(self, color, contains=None):
        super(Box, self).__init__('box', color=color)
    ...
    def toggle(self, env, pos):
        ...
        i, j = pos
        env.grid.set(i, j, self.contains)
        return True
...
class Grid:
    ...
    def horz_wall(self, x, y, length=None, object_type=Wall):
        if length is None:
            length = self.width - x
        for i in range(0, length):
            self.set(x + i, y, object_type())

    def vert_wall(self, x, y, length=None, object_type=Wall):
        if length is None:
            length = self.height - y
        for j in range(0, length):
            self.set(x, y + j, object_type())

    def wall_rect(self, x, y, w, h, object_type=Wall, fill_in=False):
        if fill_in:
            # Fill the entire rectangle and its interior to object_type
            for i in range(y, y+h):
                self.horz_wall(x, i, w, object_type=object_type)
        else:
            # Just set the perimeter of the rectangle to the object_type
            self.horz_wall(x, y, w, object_type=object_type)
            self.horz_wall(x, y+h-1, w, object_type=object_type)
            self.vert_wall(x, y, h, object_type=object_type)
            self.vert_wall(x+w-1, y, h, object_type=object_type)
...
class MiniGridEnv(gym.Env):
    ...
    def __str__(self):
        ...
        The agent is represented by forward arrow. A grid pixel is represented by 2-character
    ...
    def _rand_choice(self, choices):
        """
        Generate random choice in list
        """
    ...
    def place_agent(
        ...
        self.start_pos = None
        pos = self.place_obj(None, top, size, max_tries=max_tries, reject_fn=reject_fn, ok_to_overlap=ok_to_overlap)
        self.start_pos = pos
    ...
    def front_pos(self):
        ...
        - MFE added this to try to not go out of bounds.
        """
        true_front_pos = self.agent_pos + self.dir_vec
        clipped_front_pos = np.clip(true_front_pos, [0, 0], [self.grid.width - 1, self.grid.height - 1])
        return clipped_front_pos
    ...
    def step(self, action):
        # print("[MiniGridEnv] action:", action, ", step count:", self.step_count)
        self.step_count += 1
        ...
        #         i, j = fwd_pos
        #         self.grid.set(i, j, self.carrying)
        ...
        if new_cell.type == 'goal':
            ...
            reward = -0.01
            # reward = -np.linalg.norm(self.goal_pos - self.agent_pos)


        # Experimental... reward function
        # reward = 0
        # if current_cell.type in ['grass', 'house']:
        #     reward -= 0.1*np.linalg.norm([self.agent_pos - self.goal_pos])
        # # if current_cell.type in ['road']:
        # #     reward = -0.02
        # # if current_cell.type in ['driveway']:
        # #     reward = -0.01
        # #     reward -= np.linalg.norm([self.agent_pos - self.goal_pos])
        # # reward = -0.01
        # else:
        #     reward -= 0.01*np.linalg.norm([self.agent_pos - self.goal_pos])
        # if new_cell.type in ['goal']:
        #     reward = 1.0
    ...
    def render(self, mode='human', close=False, show_trajectory=False):
        """
        ...
        # self.add_observation_overlay_to_renderer(r)
    ...
    def add_observation_overlay_to_renderer(self, r):
        # Highlight all cells that have been seen by the agent at any time
        if self.remember_seen_cells:
            for i in range(0, self.grid.width):
                for j in range(0, self.grid.height):
                    cell = self.grid.get(i, j)
                    if cell and cell.has_been_seen:
                        r.fillRect(
                            i * CELL_PIXELS,
                            j * CELL_PIXELS,
                            CELL_PIXELS,
        ...
        # Extra highlight to cells the agent sees right now
    ...
    def add_past_trajectory_overlay_to_renderer(self, r):
        # Highlight all cells that have been visited by the agent at any time
        if self.remember_seen_cells:
            for i in range(0, self.grid.width):
                for j in range(0, self.grid.height):
                    cell = self.grid.get(i, j)
                    if cell and cell.has_been_visited:
                        r.fillRect(
                            i * CELL_PIXELS,
                            j * CELL_PIXELS,
                            CELL_PIXELS,
